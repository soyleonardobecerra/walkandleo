...
const MUNDOS_BASE = [
    {
        ...
        nombre: 'Papá y Mamá',
        codigo: 'PAPA-MAMA',
        descripcion: 'Mundo directo para papá y mamá. Nace público y puede hacerse privado con clave.',
        capacidad: null,
        configurableClave: true
    },
    {
        id: 'familia',
        nombre: 'Familia',
        codigo: 'FAMILIA',
        descripcion: 'Mundo familiar principal para los 4 usuarios habilitados.',
        capacidad: 4,
        configurableClave: false
    }
];
...
async function mostrarApp(mundo) {
    ...
    actualizarLabelsUsuario();
    mundoActivoLbl.textContent = `🌍 ${mundo.nombre} • ${mundo.codigo || mundo.id}`;
    chat.innerHTML = '';
    primeraCarga = true;
    mostrarContadorNuevos(0);

    $('btn-world-settings').style.display = mundo.id === 'papa-mama' ? 'inline-flex' : 'none';
...
async function renderMundos() {
    mundosBody.innerHTML = '<div class="empty">Cargando mundos...</div>';
    const mundos = [];

    for (const base of MUNDOS_BASE) {
        const snap = await getDoc(doc(db, 'mundos', base.id)).catch(() => null);
        mundos.push({ id: base.id, ...base, ...(snap?.exists() ? snap.data() : {}) });
    }

    mundosBody.innerHTML = `
        <div class="lobby-intro">
            Entra al mundo, abre un chat directo con alguien conectado o crea un grupo familiar. Ya no se crea un mundo propio por usuario.
        </div>
        ...
        <section class="lobby-section">
            <div class="section-head">
                <div>
                    ...
                    <div class="section-sub">Chats directos y grupos creados por la familia.</div>
                </div>
        ...
        <button class="btn-accion" id="btn-abrir-unirse">📷 ENTRAR CON QR O CÓDIGO</button>
        <div class="seccion-titulo">MUNDOS DISPONIBLES</div>
    `;
    $('btn-abrir-unirse').addEventListener('click', abrirModalUnirse);
    $('btn-crear-grupo')?.addEventListener('click', abrirCrearGrupo);
    renderDirectorioUsuarios();
    renderChatsLobby();
    mundos.forEach(m => mundosBody.appendChild(crearTarjetaMundo(m)));
}
...
async function prepararEntrarMundo(mundoResumen) {
    ...
    if (!snap?.exists()) {
        ...
        try {
            ...
            alert('No pude crear este mundo en Firebase. Crea manualmente los documentos mundos/papa-mama y mundos/familia, o revisa que las reglas permitan crear mundos base.');
            return;
        }
    }

    if (!snap?.exists()) {
        alert('Este mundo aún no existe en Firebase. Vuelve a intentar.');
        return;
    }
    ...
    if (!yaEsMiembro && mundo.capacidad && miembros.length >= mundo.capacidad) {
        alert('Este mundo ya alcanzó el límite de usuarios habilitados.');
        return;
    }
...
async function entrarMundo(mundo) {
    ...
    if (!miembros.includes(currentUser.uid)) {
        try {
            ...
            alert('No pude agregarte como miembro del mundo. Esto suele pasar cuando el mundo fue creado vacío o las reglas no permiten la primera unión. Prueba una vez más; si continúa, crea los mundos base desde Firebase con ownerUid y miembros.');
            return;
        }
    }
...
$('btn-confirmar-unirse').addEventListener('click', async () => {
    ...
    if (!mundo) {
        modalError.textContent = 'Código no encontrado. Usa PAPA-MAMA, FAMILIA o escanea el QR.';
        return;
    }
    cerrarModalUnirse();
...
async function registrarUsuarioGlobal(online = true) {
    ...
    // Fallback y compatibilidad: el directorio también queda en el mundo Familia.
    try {
        await updateDoc(doc(db, 'mundos', 'familia'), {
            miembros: arrayUnion(currentUser.uid),
        ...
        console.warn('No se pudo guardar usuario en fallback Familia:', err);
    }
    return ok;
}
...
function escucharDirectorioUsuarios() {
    ...
    try {
        unsubDirectorio = onSnapshot(collection(db, 'usuarios'), snapshot => {
            ...
            console.warn('Directorio global no disponible; usando Familia:', err);
            escucharDirectorioFallbackFamilia();
        });
    } catch (err) {
...
function escucharDirectorioFallbackFamilia() {
    if (unsubDirectorio) { unsubDirectorio(); unsubDirectorio = null; }
    unsubDirectorio = onSnapshot(doc(db, 'mundos', 'familia'), snap => {
        const mapa = new Map();
        const data = snap.exists() ? snap.data() : {};
        Object.values(data.miembrosInfo || {}).forEach(raw => {
        ...
        console.warn('Directorio fallback no disponible:', err);
        directorioUsuarios = [perfilActualComoUsuario(true)];
        renderDirectorioUsuarios();
        renderGroupUsersList();
    });
}
...
function renderChatsLobby() {
    ...
    chatsLobby.forEach(chatItem => {
        ...
        row.innerHTML = `
            ...
            <div class="user-main">
                <div class="user-name">${escapeHTML(chatItem.nombre || 'Chat familiar')}</div>
...
async function crearGrupoPersonalizado() {
    ...
    const data = {
        ...
        descripcion: 'Grupo creado desde el lobby familiar.',
        tipo: 'grupo',
        ownerUid: currentUser.uid,
        ownerEmail: currentUser.email,
        miembros,
...
$('btn-world-settings').addEventListener('click', () => {
    if (!mundoActivo || mundoActivo.id !== 'papa-mama') return;
    solicitarPinAdulto(() => abrirAjustesMundo(mundoActivo));
});
...
async function abrirAjustesMundo(mundo) {
    ...
    inpWorldName.value = data.nombre || 'Papá y Mamá';
    inpWorldPass.value = '';
...
$('btn-save-world-settings').addEventListener('click', async () => {
    ...
    if (!worldId) return;
    const nombreMundo = inpWorldName.value.trim().slice(0, 32) || 'Papá y Mamá';
