...
async function renderMundos() {
    ...
    mundosBody.innerHTML = `
        <div class="lobby-intro">
            Entra al mundo que necesites. Ya no se crea un mundo propio por usuario: todos llegan primero a este lobby.
        </div>
...
async function entrarMundo(mundo) {
    ...
    try {
        await setDoc(doc(db, 'mundos', mundo.id, 'miembros', currentUser.uid), {
            uid: currentUser.uid,
        ...
        console.error('crear perfil de miembro:', err);
        alert('Entraste al mundo, pero no pude guardar tu perfil de miembro. Revisa permisos de la subcolección miembros.');
    }
...
async function resolverMundoPorCodigo(valor) {
    ...
    for (const base of MUNDOS_BASE) {
        if (normal === base.id.toUpperCase() || normal === base.codigo.toUpperCase()) {
            const snap = await getDoc(doc(db, 'mundos', base.id)).catch(() => null);
            return { ...base, ...(snap?.exists() ? snap.data() : {}) };
        }
    }
...
async function actualizarPerfilEnMundoActivo() {
    ...
    const perfil = perfilUsuario();
    await setDoc(doc(db, 'mundos', mundoActivo.id, 'miembros', currentUser.uid), {
        uid: currentUser.uid,
        email: currentUser.email,
        displayName: perfil.displayName,
        avatar: perfil.avatar,
...
function detenerPresencia() {
    ...
    if (currentUser && mundoActivo) {
        setDoc(doc(db, 'mundos', mundoActivo.id, 'miembros', currentUser.uid), {
            lastSeen: Date.now(),
            isOnline: false
        }, { merge: true }).catch(() => null);
    }
