...
// App Check:
// 1) En Firebase Console > App Check, registra esta app web.
// 2) Crea/usa una clave de reCAPTCHA Enterprise.
// 3) Reemplaza el texto de abajo por tu site key.
// 4) Cuando confirmes que todo funciona, activa enforcement para Firestore y Storage.
const APP_CHECK_SITE_KEY = "PEGA_AQUI_TU_SITE_KEY_DE_RECAPTCHA_ENTERPRISE";
...
let globalPresenceIntervalId = null;
    ...
    mundosBody.innerHTML = `
        <section class="lobby-section">
            <div class="section-head">
                <div>
                    <div class="seccion-titulo">USUARIOS REGISTRADOS</div>
                    <div class="section-sub" id="usuarios-count">Online/offline en tiempo real.</div>
                </div>
            </div>
...
async function prepararEntrarMundo(mundoResumen) {
    if (!currentUser || !mundoResumen?.id) return;

    const refMundo = doc(db, 'mundos', mundoResumen.id);
    const snap = await getDoc(refMundo).catch(() => null);

    if (!snap?.exists()) {
        alert('Este chat o grupo ya no existe en Firebase.');
        return;
    }
...
async function entrarMundo(mundo) {
    ...
    if (!miembros.includes(currentUser.uid)) {
        alert('No perteneces a este chat o grupo. Pide que te creen o te agreguen al grupo.');
        return;
    }
...
function iniciarPresenciaGlobal() {
    ...
    registrarUsuarioGlobal(!document.hidden).catch(() => null);
    globalPresenceIntervalId = setInterval(() => {
        if (!currentUser) return;
        registrarUsuarioGlobal(!document.hidden).catch(() => null);
    }, 30 * 1000);
}

function detenerPresenciaGlobal() {
    if (globalPresenceIntervalId) {
        clearInterval(globalPresenceIntervalId);
        globalPresenceIntervalId = null;
    }
}
...
function renderDirectorioUsuarios() {
    ...
    lista.forEach(u => {
        ...
        `;
        row.style.cursor = esYo ? 'default' : 'pointer';
...
async function abrirChatDirecto(usuario) {
    ...
    const refChat = doc(db, 'mundos', id);

    const existente = await getDoc(refChat).catch(() => null);
    if (existente?.exists()) {
        return entrarMundo({ id, ...existente.data() });
    }
    ...
    const data = {
        nombre: `Chat con ${displayNameDe(otro)}`,
    ...
    try {
        await setDoc(refChat, data);
        await entrarMundo({ id, ...data });
    } catch (err) {
        console.warn('No se pudo crear chat directo:', err);
        alert('No pude abrir el chat directo. Revisa las reglas de Firebase.');
    }
}
...
async function subirAudio() {
    ...
    try {
        ...
        const ruta = `audios/${mundoActivo.id}/${currentUser.uid}/${Date.now()}.${ext}`;
...
async function limpiarAudiosViejos(mundoId) {
    try {
        ...
        for (const d of snap.docs) {
            const data = d.data();
            // Desde el cliente solo borramos mensajes propios vencidos.
            // Para limpieza global de todos los usuarios, usar Cloud Functions.
