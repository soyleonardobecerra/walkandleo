    ...
    for (const base of MUNDOS_BASE) {
        ...
        if (!snap?.exists()) {
            await setDoc(refMundo, {
                ...
                creadoAt: Date.now(),
                updatedAt: Date.now()
            }).catch(err => console.warn('crear mundo base:', err));
        } else {
            // Actualiza metadatos sin tocar clave ni miembros.
            await setDoc(refMundo, {
                codigo: snap.data().codigo || base.codigo,
                descripcion: snap.data().descripcion || base.descripcion,
                tipo: 'base',
                capacidad: base.capacidad,
                configurableClave: base.configurableClave,
                updatedAt: Date.now()
...
async function prepararEntrarMundo(mundoResumen) {
    ...
    const snap = await getDoc(doc(db, 'mundos', mundoResumen.id)).catch(() => null);
    if (!snap?.exists()) {
        alert('Este mundo aún no existe en Firebase. Revisa las reglas o vuelve a intentar.');
        return;
    }
